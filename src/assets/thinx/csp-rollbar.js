/* Rollbar for the public (pre-login) pages.

The AngularJS app at app/index.html gets Rollbar through the ng-rollbar module,
which never loads here -- login, OAuth return, password reset and the error page
are plain static pages, so uncaught errors in those flows went unreported.

This is the official rollbar.js v3.1.0 snippet from
https://cdn.rollbar.com/rollbarjs/refs/tags/v3.1.0/rollbar.snippet.js
kept in its own file rather than inline: the CSP has no 'unsafe-inline' in
script-src and test/csp/source-check.cjs fails the build on inline scripts.

Load it FIRST in <head> so it is installed before the scripts it should watch.
*/
( function() {

  // Same guard and the same reasons as app/js/main.js: gulp-inject-envs writes
  // the literal string "undefined" when ROLLBAR_ACCESS_TOKEN is missing, and
  // leaves the injection placeholder untouched in unbuilt sources. Length is
  // deliberately not pinned -- the THiNX projects use 32 and 96 hex characters.
  // deepcode ignore HardcodedNonCryptoSecret: client-side post_client_item token, injected at build time
  var accessToken = "<ENV::rollbarAccessToken>";
  var environment = "<ENV::environment>";
  var codeVersion = "<ENV::versionCode>";

  if ( !/^[0-9a-f]{32,}$/i.test( accessToken ) ) {
    // Loud on purpose: staying silent here is what hid Rollbar being off.
  ...
  window._rollbarConfig = {
    ...
    // Appended to Rollbar's own defaults (no overwriteScrubFields here). These
    // pages carry credentials, so the list matters more than it does in the app.
    scrubFields: [
      "api_key",
      "apikey",
      "authorization",
    ...
    // Login forms are the reason this is on: never record what was typed.
    scrubTelemetryInputs: true,
    payload: {
      environment: /^[a-z]+$/.test( environment ) ? environment : "development",
      client: {
        javascript: {
          code_version: /^[0-9a-f]{4,40}$/i.test( codeVersion ) ? codeVersion : undefined,
