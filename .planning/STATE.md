...
- **Next action:** v1.0 GA close-out is one step away from "fully done". Three independent threads remain, all carry-overs from Phase 9 (none are Phase 11 work): (a) **DASH-04 download walk** — postponed; needs identifying a device with a successful firmware build artifact in the production filesystem (qualifying device currently unknown — lookup needed on swarm host `188.166.23.244`). (b) **AUTH-03 laptop-sleep walk** — pending on a real-user idle session long enough for the JWT `exp` claim to fire. (c) **`AUTH-bearer-null` v1.x cleanup** — hygiene fix on `vue/src/core/api.js:53-57` (don't send `Bearer null` header when logged out); harmless under the parent's class-fix guard, can ship in v1.1. None of these blocks v1.0 GA from this submodule — they are post-GA polish. Optional immediate next move: `/gsd:complete-milestone` to archive v1.0 and prep v1.1, OR park here until a Phase 9 walk window opens.
