...
- **Next action:** **G9 live UAT** — walk the ROADMAP.md L393 scenario on `console.thinx.cloud` (`/#/app/devices` → tick row → per-row Revoke → confirm → device disappears AND toolbar counter drops to `(0)`). After that walk passes, Phase 11 in this submodule can be marked Verified and v1.0 GA from the console side is locked in. Side tracks: (a) **DASH-04 download walk POSTPONED** — requires identifying a device that has a successful firmware build available in the production filesystem; the qualifying device is currently unknown and must be looked up on the swarm host before the walk can run. (b) AUTH-03 laptop-sleep walk still pending on external state. v1.x backlog updated 2026-05-27 with `AUTH-bearer-null` (Vue console sending `Bearer null` header on logged-out requests — harmless under parent's class-fix backend guard but worth a hygiene cleanup). Parent-monorepo GSD project owns the remaining v1 GA backend phases (SEC-PII-01, OPS-01, SEC-DEP-01).
