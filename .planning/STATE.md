...
- **Next action:** v1.0 GA close-out from this submodule is fully done. Two threads remain, none blocking v1.0: (a) **DASH-04 zip-download walk — BLOCKED on G10 worker fix** per 2026-05-27 production-fs audit on swarm `188.166.23.244` (`/mnt/gluster/thinx/deploy`: 217 owner dirs, 7 build.log files, 6 FAILED + 1 aborted, 0 `firmware.bin` artifacts anywhere — most-recent attempt 2023-03-07). DASH-04's widget-rendering sub-criterion is already verified; only the live download sub-criterion is blocked, and it unblocks together with G10. Filed as `OPS-builder-broken` in `.planning/v1.x-backlog.md`. (b) **AUTH-03 laptop-sleep walk** — pending on a real-user idle session long enough for the JWT `exp` claim to fire. (c — backlog only) `AUTH-bearer-null` Vue hygiene cleanup. Optional immediate next move: `/gsd:complete-milestone` to archive v1.0 and prep v1.1, OR park here until AUTH-03 walk window opens.
